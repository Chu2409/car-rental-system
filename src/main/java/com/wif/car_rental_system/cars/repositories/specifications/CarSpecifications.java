    ...
    public static Specification<CarEntity> withFilters(CarFilters filters) {
        return (root, query, criteriaBuilder) -> {
            ...
            if (filters.getBrand() != null && !filters.getBrand().isEmpty()) {
                predicates = criteriaBuilder.and(predicates, criteriaBuilder.like(root.get("brand"), "%" + filters.getBrand() + "%"));
            }

            if (filters.getMinPrice() != null) {
                predicates = criteriaBuilder.and(predicates, criteriaBuilder.greaterThanOrEqualTo(root.get("dailyRate"), filters.getMinPrice()));
            }

            if (filters.getMaxPrice() != null) {
                predicates = criteriaBuilder.and(predicates, criteriaBuilder.lessThanOrEqualTo(root.get("dailyRate"), filters.getMaxPrice()));
            }
